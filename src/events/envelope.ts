...
// batch.received (with lagMs measured from the enqueue time of the oldest message in the batch) and
...
export type Event =
  | { type: 'batch.received'; queue: string; size: number; lagMs: number; at: number }
  | { type: 'message.started'; id: string; attempts: number; at: number }
  | { type: 'message.succeeded'; id: string; durationMs: number; at: number }
  | {
      type: 'message.failed';
      id: string;
